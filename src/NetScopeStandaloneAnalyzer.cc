...
void NetScopeStandaloneAnalyzer::InitLoop(){
  ...
  if(!skip_tracks && pixel_input_file_path != ""){
    ...
    tree->Branch("xErr04", &xErr04, "xErr04/F");
    tree->Branch("yErr05", &yErr05, "yErr05/F");
    tree->Branch("xResid04", &xResid04, "xResid04/F");
    tree->Branch("yResid05", &yResid05, "yResid05/F");
    ...
    while (idx_px_tree < entries_px_tree && i_evt >= (pixel_event->trigger+0)) {
      ...
      if ((pixel_event->trigger+0) == i_evt) {
        if(ntracks == 0) {
          ...
          xErr04=pixel_event->xErr04;
          yErr05=pixel_event->yErr05;
          xResid04=pixel_event->xResid04;
          yResid05=pixel_event->yResid05;
